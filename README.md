# Running ROSMower in Gazebo Simulation in Windows

This Dockerfile will install rOSMower and all related OpenMower components.
As base, it uses this docker container https://github.com/jbnunn/ROSGazeboDesktop

To install:

  
  
    docker build -t paddycube/rosmower:dev .
...
### Windows
    docker run -it --rm --name=ros_gazebo_desktop -m=4g -p 6080:80 -p 5900:5900 -v %cd%/data:/home/ubuntu/data -e RESOLUTION=1920x1080 -e USER=ubuntu -e PASSWORD=ubuntu paddycube/rosmower:simulation 
    
### OS X / Linux:

    docker run -it --rm --name=ros_gazebo_desktop -m=4g -p 6080:80 -p 5900:5900 -v $PWD/data:/home/ubuntu/data -e RESOLUTION=1920x1080 -e USER=ubuntu -e PASSWORD=ubuntu paddycube/rosmower:simulation   
